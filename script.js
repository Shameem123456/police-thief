...
// UI elements for word input (we'll hide/show them)
...
window.onload = function () {
    ...
    gameWordInput.addEventListener('input', function() {
        // Only enable if it's my turn and input not empty (visibility handled elsewhere)
...
async function submitWord() {
    ...
    // Immediately hide the input and button after click
    ...
    // No need to clear value here – input is hidden
    // Button will be re‑enabled when turn changes (handled by syncState)
    // The input will be shown again when it's our turn (in updateGameScreen)
}
...
function handlePhaseChange(data) {
    ...
    if (phase === 'REVEAL') {
        ...
        // Ensure input and button are visible when entering game (if it's our turn, updateGameScreen will handle)
...
function updateGameScreen(data) {
    ...
    // Control visibility of word input based on turn
    if (isMyTurn) {
        // It's our turn – show the input and button (they may have been hidden after submit)
        gameWordInput.style.display = '';
        submitWordBtn.style.display = '';
        gameWordInput.disabled = false;
        // Enable button only if input not empty
        submitWordBtn.disabled = gameWordInput.value.trim() === '';
        submitWordBtn.classList.remove('btn-loading'); // remove loading if stuck
    } else {
        // Not our turn – hide the input and button completely
        gameWordInput.style.display = 'none';
        submitWordBtn.style.display = 'none';
