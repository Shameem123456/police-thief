...
// UI elements for word input
let gameWordInput, submitWordBtn;

// Turn tracking for hiding input until next actual turn
let myTurnActive = false;      // true only when it's our turn and we haven't submitted yet
let prevIsMyTurn = false;      // used to detect turn start
...
window.onload = function () {
    ...
    gameWordInput = document.getElementById('game-word');
    submitWordBtn = document.getElementById('btn-submit-word');
...
async function submitWord() {
    ...
    // Immediately hide input and button – they will stay hidden until next actual turn
    gameWordInput.style.display = 'none';
    submitWordBtn.style.display = 'none';
    myTurnActive = false;  // we've submitted, so our turn is effectively over
    ...
    // No need to clear value – input hidden
}
...
function handlePhaseChange(data) {
    ...
    if (phase === 'REVEAL') {
        ...
        // Reset turn tracking when entering game
        myTurnActive = false;
        prevIsMyTurn = false;

        // Ensure input and button are visible (they will be controlled by updateGameScreen)
        gameWordInput.style.display = '';
        submitWordBtn.style.display = '';
    } else if (phase === 'VOTE') {
...
function updateGameScreen(data) {
    ...
    // Detect when our turn actually starts (transition from false to true)
    if (isMyTurn && !prevIsMyTurn) {
        myTurnActive = true;               // we can now type
        // Also ensure the input is visible (will be shown below)
    }

    // Control visibility based on turn and whether we have already submitted
    if (isMyTurn && myTurnActive) {
        // It's our turn and we haven't submitted yet – show input
        gameWordInput.style.display = '';
        submitWordBtn.style.display = '';
        gameWordInput.disabled = false;
        submitWordBtn.disabled = gameWordInput.value.trim() === '';
        submitWordBtn.classList.remove('btn-loading');
    } else {
        // Not our turn, or we already submitted – hide input
        gameWordInput.style.display = 'none';
        submitWordBtn.style.display = 'none';
    ...
    // Store current turn state for next comparison
